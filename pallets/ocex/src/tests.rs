...
use polkadex_primitives::{
	ocex::AccountInfo, snapshot::EnclaveSnapshot, AccountId, AssetsLimit, Balance, ProxyLimit,
	WithdrawalLimit,
};
...
fn test_withdrawal_invalid_withdrawal_index() {
	...
	new_test_ext().execute_with(|| {
		assert_noop!(
			OCEX::withdraw(Origin::signed(account_id.clone().into()), 1, 1),
...
fn test_withdrawal() {
	...
	t.execute_with(|| {
		...
		assert_ok!(OCEX::withdraw(Origin::signed(account_id.clone().into()), 0, 0));
	});
}
...
fn test_withdrawal_bad_origin() {
	let account_id = create_account_id();
	new_test_ext().execute_with(|| {
		assert_noop!(OCEX::withdraw(Origin::root(), 1, 1), BadOrigin);

		assert_noop!(OCEX::withdraw(Origin::none(), 1, 1), BadOrigin);
	});
}
...
pub fn create_withdrawal<T: Config>() -> Withdrawal<AccountId32, BalanceOf<T>> {
	...
	let withdrawal: Withdrawal<AccountId32, BalanceOf<T>> =
		Withdrawal { main_account: account_id, asset: AssetId::polkadex, amount: 0_u32.into() };
	return withdrawal
}
